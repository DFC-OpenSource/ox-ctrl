 ...
 * 
 * WARNING: THIS MEDIA MANAGER NEEDS SOME FIXES MARKED AS 'TODO' for OX 2.x
 */
...
static void dfcnand_oob_reorder (uint8_t *oob, uint32_t map)
{
    int i;
    uint8_t oob_off = 0;
    uint32_t meta_sz = NAND_EXPOSED_OOB;

    for (i = 0; i < NAND_SECTOR_COUNT - 1; i++)
        if (map & (1 << i))
            memcpy (oob + oob_off,
                    oob + oob_off + NAND_EXPOSED_SEC_OOB,
                    meta_sz - oob_off - NAND_EXPOSED_SEC_OOB);
        else
            oob_off += NAND_EXPOSED_SEC_OOB;
}
...
static int dfcnand_dma_helper (io_cmd *cmd)
{
    ...
    for (; c < nvm_cmd->n_sectors; c++) {
        ...
        ret = ox_dma ((void *)(cmd->dfc_io.virt_addr + nvm_cmd->sec_sz * c),
...
OOB:
    ...
    if ((cmd->dfc_io.local_dma & DFCNAND_LS2_DMA_OOB) && nvm_cmd->md_prp)
        if(ox_dma ((void *)(oob_addr), nvm_cmd->md_prp,
...
static int dfcnand_start_mq (void)
{
    ...
    mq_config.to_usec = DFCNAND_QUEUE_TO;
    mq_config.flags = 0;
...
static int dfcnand_read_page (struct nvm_mmgr_io_cmd *cmd_nvm)
{
    ...
    for (c = 0; c < 5; c++) {
        ...
        cmd->host_addr[c] = (cmd->dfc_io.local_dma & dma_flag) ?
            ...
            0;//(uint64_t) core.nvm_pcie->host_io_mem->paddr + prp_sec[c];
        /* TODO: host_io_mem not present in OX 2.x. Adapt the line above */
...
int mmgr_dfcnand_init (void)
{
    ...
    return ox_register_mmgr(&dfcnand);
}
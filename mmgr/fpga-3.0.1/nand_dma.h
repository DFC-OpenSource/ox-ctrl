...
#include<time.h>
#include<sys/time.h>
/* M */#include"../../include/ox-uatomic.h"
/* N */#include "fpga-3_0_1.h"

/*Uncomment this to enable Interrupt, the interrupt support is added in
 *                                                          03.00.01 iSSD SDK*/
#define INTR
...
#define NAND_PAGES_PER_BLOCK 512
/* M */#define NAND_BLOCK_COUNT /*2096*/ 1024
#define NAND_PLANE_COUNT 2
#define NAND_TARGET_COUNT 2
#define NAND_LUN_COUNT  2
#define NAND_CHIP_COUNT  8
...
typedef struct nand_cmd_struct {
    ...
    uint32_t len3_len2;
    uint32_t oob_LSB;
    uint32_t oob_len_MSB; /*contains MSB of oob in 1st 20 bits and
                           *                        oob-len in next 12 bits*/
...
typedef struct io_cmd {
    ...
    /* N */ struct nand_cmd_struct nand_cmd;
} io_cmd;
...
typedef struct issd_ftl {
    uint8_t channel;
    uint16_t block;
    uint16_t page;
} issd_ftl_t;

uint8_t make_desc(nand_cmd_struct *cmd_struct, uint8_t tid,
                                            uint8_t req_type, void *req_ptr);
...
/* M */int dfcnand_callback(io_cmd *cmd);

uint8_t nand_dm_init (void);
uint8_t nand_dm_deinit (void);

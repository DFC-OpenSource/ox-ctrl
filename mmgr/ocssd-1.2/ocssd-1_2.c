                       ...
                       struct nvm_addr *addr, uint64_t *prp, uint64_t *md_prp)
{
    ...
    for (int sec = 0; sec < cmd->n_sectors; sec++) {
        ...
        printf ("  (%d) ch: %d, lun: %d, blk: %d, pl: %d, pg: %d, sec %d, "
                "prp: %lx, md: %lx, md_low: %lx, md_high: %lx\n",
                sec, addr[sec].g.ch, addr[sec].g.lun, addr[sec].g.blk,
                addr[sec].g.pl, addr[sec].g.pg, addr[sec].g.sec, prp[sec],
...
static void ocssd_process_sq (struct ox_mq_entry *req)
{
    ...
    if (cmd->cmdtype == MMGR_ERASE_BLK) {
        ...
        for (int pl = 0; pl < ocssd.mmgr_geo.n_of_planes; pl++) {
            addrs[pl].g.ch = cmd->ppa.g.ch;
            addrs[pl].g.lun = cmd->ppa.g.lun;
            addrs[pl].g.blk = cmd->ppa.g.blk;
            addrs[pl].g.pl = pl;
        }
        goto EXEC;
    }
    ...
    for (int sec = 0; sec < cmd->n_sectors; sec++) {
        if (!cmd->prp[sec] && cmd->cmdtype == MMGR_WRITE_PG) {
            ...
            addrs[sec].g.ch = cmd->ppa.g.ch;
            addrs[sec].g.lun = cmd->ppa.g.lun;
            addrs[sec].g.blk = cmd->ppa.g.blk;
            addrs[sec].g.pg = cmd->ppa.g.pg;
            addrs[sec].g.pl = cmd->ppa.g.pl;
            addrs[sec].g.sec = sec % ocssd.mmgr_geo.sec_per_pg;

            if (cmd->n_sectors > ocssd.mmgr_geo.sec_per_pg)
                  addrs[sec].g.pl = sec / ocssd.mmgr_geo.sec_per_pg;
        }
    }
...
EXEC:
    ...
    switch (cmd->cmdtype) {
        case MMGR_READ_PG:
            ...
            if (nvm_addr_read (ocssd.dev, addrs, cmd->n_sectors,
                        (void *) prp[0], (void *) md_prp[0], PMODE, &ret)) {

                /* Flash page hasn't been programmed, return zeros */
                if (ret.result == 0x2ff && (ret.status == 0x1 || ret.status == 0xff) ) {
                    ...
                    log_info ("[ocssd: READ ERROR %x/%lx\n", ret.result, ret.status);
                    goto ERR;
                }
        ...
        case MMGR_WRITE_PG:
            if (nvm_addr_write (ocssd.dev, addrs, cmd->n_sectors,
        ...
        case MMGR_ERASE_BLK:
            if (nvm_addr_erase (ocssd.dev, addrs, ocssd.mmgr_geo.n_of_planes,
                                                                  PMODE, &ret))
                goto ERR;
...
static int ocssd_init_geo (void)
{
    ...
    g->n_of_ch      = ocssd.geo->nchannels;
    g->lun_per_ch   = ocssd.geo->nluns;
    g->blk_per_lun  = ocssd.geo->nblocks + OCSSD_BLK_BOUND;
    g->pg_per_blk   = ocssd.geo->npages;
    g->sec_per_pg   = ocssd.geo->nsectors;
    g->n_of_planes  = ocssd.geo->nplanes;
    g->pg_size      = ocssd.geo->page_nbytes;
    g->sec_oob_sz   = ocssd.geo->meta_nbytes;
